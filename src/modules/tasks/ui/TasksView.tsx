...
type TaskForm = { concept: string; details: string; assignedTo: string[] };
...
export function TasksView({ addTask, currentUserId, isAdmin, onRemoveTask, onUpdateTask, onToggleTask, setTaskForm, taskForm, tasks, userById, users }: TasksViewProps) {
  ...
  return (
    <section className="stack">
      <section className="card">
        <div className="card-title"><div><h2>Tareas</h2><span>Organizá pendientes y tareas completadas</span></div><button className="primary-btn" onClick={() => setIsModalOpen(true)}><Plus size={18} /> Crear tarea</button></div>
      </section>
      <div className="tasks-grid">
        <TaskTable title="No realizadas" tasks={pending} onRemoveTask={onRemoveTask} onUpdateTask={onUpdateTask} onToggleTask={onToggleTask} userById={userById} />
        <TaskTable title="Realizadas" tasks={completed} onRemoveTask={onRemoveTask} onUpdateTask={onUpdateTask} onToggleTask={onToggleTask} userById={userById} />
      </div>
      {isModalOpen && (
...
function TaskTable({ title, tasks, onRemoveTask, onUpdateTask, onToggleTask, userById }: { title: string; tasks: Task[]; onRemoveTask: TasksViewProps['onRemoveTask']; onUpdateTask: TasksViewProps['onUpdateTask']; onToggleTask: TasksViewProps['onToggleTask']; userById: TasksViewProps['userById'] }) {
  return (
    <section className="card task-panel">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="table-list">
          {tasks.map((task) => (
            <TaskRow key={task.id} onRemoveTask={onRemoveTask} onUpdateTask={onUpdateTask} onToggleTask={onToggleTask} task={task} userName={userById(task.ownerId)} />
          ))}
        </div>
      )}
    </section>
...
function TaskRow({ onRemoveTask, onUpdateTask, onToggleTask, task, userName }: { onRemoveTask: TasksViewProps['onRemoveTask']; onUpdateTask: TasksViewProps['onUpdateTask']; onToggleTask: TasksViewProps['onToggleTask']; task: Task; userName: string }) {
  ...
  return (
    <article className="task-row">
      ...
      <div><strong>{task.concept}</strong><em className="task-owner">{userName}</em>{task.details && <p>{task.details}</p>}</div>
